...
import { MilestoneEventApiUrl} from './apiUrlConst/MilestoneEventApiUrls';
...
@Injectable()
export class MilestoneEventService {
    ...
    postGridData(): Observable<any> {
        return this.http.post(`${MilestoneEventApiUrl.gridApiUrl}`, this.body, { headers: this.headers })
            .map(this.getJson);
        //this.checkErrors)
    ...
    postGridDataFilter(filterBody): Observable<any> {
        ...
        return this.http.post(`${MilestoneEventApiUrl.gridApiUrl}`, filterBody, { headers: this.headers })
            .map(this.getJson);
        //this.checkErrors)
        //.catch(err => Observable.throw(err))
        //.map(this.getJson);
    ...
    postUpdate(filterBody): Observable<any> {
        console.log("-------- Post Customers FilterBody --------", filterBody);
        return this.http.put(`${MilestoneEventApiUrl.postUpdateUrl}`, filterBody, { headers: this.headers })
            .map(this.getJson)
    ...
    getById(id): Observable<any> {
        return this.http.get(`${MilestoneEventApiUrl.getByIdUrl}/${id}`, { headers: this.headers })
            .map(this.getJson)
            ;
        //.catch(err => Observable.throw(err))
        //.map(this.getJson);
